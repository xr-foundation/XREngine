...
import { createFeathersKoaApp } from '@xrengine/server-core/src/createApp'
...
export const start = async (): Promise<void> => {
  ...
  if (!config.kubernetes.enabled) {
    ...
    if (!databaseService) {
      ...
      //     throw new Error('\x1b[33mError: DB process is not running or Docker is not running!. If you are in local development, please run xrengine/scripts/start-containers.sh and restart server\x1b[0m');
      //   }
      // });
    }
  }
