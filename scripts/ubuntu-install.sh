...
# - The primary target user of this script is a non-technical user that is running XREngine for their first time.
...
# @fileoverview
# Easy XREngine Installer for localdev on Ubuntu.
# Manages installation of all of XREngine's dependencies for the first time.
...
# - Clones XREngine's main repository (@important does NOT use --depth=1)
...
if [[ $(lsb_release -si) -ne "Ubuntu" ]] ; then
  ...
  case "$choice" in
    y|Y) warn "Running XREngine Ubuntu installer script on:"; echo "  $(lsb_release -sa)";;
    n|N|*) exit 1;;
  esac
fi
...
echo "5. Clone XREngine's repository to the folder $(pwd)/xrengine"
echo "6. Install all npm dependencies that XREngine needs"
echo ""
read -r -n1 -p 'Do you want to continue? [y/n] ' choice
case "$choice" in
  y|Y) info "Running XREngine's Ubuntu installer" ;;
  n|N|*) exit 1 ;;
esac
...
# Install the engine
info "Installing XREngine into folder $(pwd)/xrengine ..."
git clone https://github.com/xr-foundation/xrengine
info "Changing folder to $(pwd)/xrengine"
cd xrengine
...
npm install
echo ""
info "XREngine has been successfully installed into $(pwd)."
