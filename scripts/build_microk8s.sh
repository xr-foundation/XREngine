...
if [ -z "$VITE_APP_HOST" ]; then
  VITE_APP_HOST=local.xrfoundation.org
else
  VITE_APP_HOST=$VITE_APP_HOST
fi

if [ -z "$VITE_SERVER_HOST" ]; then
  VITE_SERVER_HOST=api-local.xrfoundation.org
else
  VITE_SERVER_HOST=$VITE_SERVER_HOST
fi

if [ -z "$VITE_FILE_SERVER" ]; then
  VITE_FILE_SERVER=https://localhost:9000/xrengine-microk8s-static-resources
else
...
if [ -z "$VITE_INSTANCESERVER_HOST" ]; then
  VITE_INSTANCESERVER_HOST=instanceserver-local.xrfoundation.org
else
...
if [ -z "$VITE_AVATURN_URL" ]; then
  VITE_TERMS_OF_SERVICE_ADDRESS=https://www.xrfoundation.org/terms-of-service
else
...
docker buildx build \
  ...
  -t $REGISTRY_HOST:32000/xrengine \
...
docker tag $REGISTRY_HOST:32000/xrengine $REGISTRY_HOST:32000/xrengine:$TAG
docker push $REGISTRY_HOST:32000/xrengine:$TAG

# DOCKER_BUILDKIT=1 docker build -t $REGISTRY_HOST:32000/xrengine-testbot -f ./dockerfiles/testbot/Dockerfile-testbot .

# docker tag $REGISTRY_HOST:32000/xrengine-testbot $REGISTRY_HOST:32000/xrengine-testbot:$TAG
# docker push $REGISTRY_HOST:32000/xrengine-testbot:$TAG
