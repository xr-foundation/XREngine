  ...
  try {
    ...
    if (options.service !== 'builder') {
      ...
      if (options.service === 'instanceserver') {
        ...
        currentImages = matchingPods.map(
          (item) => item.spec.containers.find((container) => container.name === 'xrengine').image.split(':')[1]
        )
      }
    ...
    const images = await getAllImages(ecr, options.repoName || 'xrengine', undefined, [], options.public === true)
    if (!images) return
